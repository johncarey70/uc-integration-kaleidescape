...
class KaleidescapePlayer:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._emit_update(EntityPrefix.MEDIA_PLAYER.value, self.device_id, {MediaAttr.STATE: self.device.power.state})
    ...
    async def _handle_connected(self):
        ...
        #self.events.emit(Events.UPDATE.name, self.device_id, {MediaAttr.STATE: self.device.power.state})
    ...
    async def _handle_events(self, event: str):
        ...
        await self._emit_update(EntityPrefix.MEDIA_PLAYER.value, MediaAttr.MEDIA_IMAGE_URL, self.device.movie.cover)
        await self._emit_update(EntityPrefix.MEDIA_PLAYER.value, MediaAttr.MEDIA_TITLE, self.device.movie.title)
        await self._emit_update(EntityPrefix.MEDIA_PLAYER.value, MediaAttr.MEDIA_TYPE, self.device.movie.media_type)
        await self._emit_update(EntityPrefix.MEDIA_LOCATION.value, SensorAttr.VALUE, self.device.automation.movie_location)
        await self._emit_update(EntityPrefix.PLAY_SPEED.value, SensorAttr.VALUE, str(self.device.movie.play_speed))
        await self._emit_update(EntityPrefix.PLAY_STATUS.value, SensorAttr.VALUE, self.device.movie.play_status)
