"""
Kaleidescape remote entity implementation.

Derived from Unfolded Circle integration framework.
...
class KaleidescapeRemote(Remote):
    ...
    def create_button_mappings(self) -> list[DeviceButtonMapping | dict[str, Any]]:
        ...
        return [
            ...
            create_btn_mapping(Buttons.MENU, cmds.MENU.display_name),
            create_btn_mapping(Buttons.STOP, cmds.STOP_OR_CANCEL.display_name),
        ]
    ...
    def create_ui(self) -> list[UiPage | dict[str, Any]]:
        ...
        ui_page1.add(
            create_ui_text(
                ...
                1,
                size=Size(3, 1),
                cmd=cmds.INTERMISSION.display_name,
            )
        )
        ...
        ui_page1.add(
            create_ui_text(
                ...
                3,
                size=Size(2, 1),
                cmd=cmds.MOVIE_COLLECTIONS.display_name,
            )
        )
        ui_page1.add(
            create_ui_text("Covers", 2, 3, size=Size(2, 1), cmd=cmds.MOVIE_COVERS.display_name)
        )
        ui_page1.add(
            create_ui_text("List", 4, 3, size=Size(2, 1), cmd=cmds.MOVIE_LIST.display_name)
        )
        ui_page1.add(
            create_ui_text(
                "Alphabetize",
                ...
                4,
                size=Size(2, 1),
                cmd=cmds.ALPHABETIZE_COVER_ART.display_name,
            )
        )
        ui_page1.add(
            create_ui_text(
                ...
                4,
                size=Size(2, 1),
                cmd=cmds.SHUFFLE_COVER_ART.display_name,
            )
        )
        ui_page1.add(
            create_ui_text("Store", 4, 4, size=Size(2, 1), cmd=cmds.MOVIE_STORE.display_name)
        )
        ui_page1.add(
            create_ui_text("Cancel", 0, 5, size=Size(2, 1), cmd=cmds.CANCEL.display_name)
        )
        ui_page1.add(
            create_ui_text("Search", 2, 5, size=Size(2, 1), cmd=cmds.SEARCH.display_name)
        )
        ui_page1.add(
                    ...
                    if not simple_cmd:
                        ...
                        match simple_cmd:
                            case cmds.ALPHABETIZE_COVER_ART:
                                status = await self._device.send_command("alphabetize_cover_art")
                            case cmds.BACK:
                                status = await self._device.send_command("back")
                            case cmds.CANCEL:
                            ...
                            case cmds.MENU:
                                status = await self._device.send_command("menu")
                            case cmds.MOVIE_COLLECTIONS:
                                status = await self._device.send_command("collections")
                            case cmds.MOVIE_COVERS:
                                status = await self._device.send_command("movie_covers")
                            case cmds.MOVIE_LIST:
                                status = await self._device.send_command("list")
                            case cmds.MOVIE_STORE:
                                status = await self._device.send_command("movie_store")
                            case cmds.PAGE_DOWN:
                                status = await self._device.send_command("page_down")
                            case cmds.PAGE_DOWN_PRESS:
                                status = await self._device.send_command("page_down_press")
                            case cmds.PAGE_DOWN_RELEASE:
                                status = await self._device.send_command("page_down_release")
                            case cmds.PAGE_UP:
                                status = await self._device.send_command("page_up")
                            case cmds.PAGE_UP_PRESS:
                                status = await self._device.send_command("page_up_press")
                            case cmds.PAGE_UP_RELEASE:
                                status = await self._device.send_command("page_up_release")
                            case cmds.PLAY_PAUSE:
                                status = await self._device.send_command("play_or_pause")
                            case cmds.PREVIOUS:
                                status = await self._device.send_command("media_previous_track")
                            case cmds.NEXT:
                                status = await self._device.send_command("media_next_track")
                            case cmds.UP:
                                status = await self._device.send_command("cursor_up")
                            case cmds.DOWN:
                                status = await self._device.send_command("cursor_down")
                            case cmds.LEFT:
                                status = await self._device.send_command("cursor_left")
                            case cmds.RIGHT:
                                status = await self._device.send_command("cursor_right")
                            case cmds.OK:
                                status = await self._device.send_command("media_select")
                            case cmds.STOP:
                                status = await self._device.send_command("media_stop")
                            case cmds.STOP_OR_CANCEL:
                                status = await self._device.send_command("stop_or_cancel")
                            case cmds.REPLAY:
                                status = await self._device.send_command("replay")
                            case cmds.SEARCH:
                                status = await self._device.send_command("search")
                            case cmds.SHUFFLE_COVER_ART:
                                status = await self._device.send_command("shuffle_cover_art")
                            case cmds.SUBTITLES:
                                status = await self._device.send_command("subtitles")
                            case _:
                                status = StatusCodes.NOT_IMPLEMENTED
